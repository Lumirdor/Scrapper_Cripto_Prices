...
const nombreArchivo = 'precios22.csv'
    ...
    //const encabezado = 'Time,open,high,low,close,volume,closeTime,assetVolume,trades,buyBaseVolume,buyAssetVolume,ingnored\n';

console.clear();

//Funcion que agrega contenido csv al archivo. Si el archivo no existe lo crea
function grabar(contenido, nombreArchivo){
console.log('Este es el primer elemento de contenido: contenido[0]'+contenido[0]);
  fs.readFile(nombreArchivo, 'utf8', (err, data) => {
    if (err) {
      const encabezado = 'Time,open,high,low,close,volume,closeTime,assetVolume,trades,buyBaseVolume,buyAssetVolume,ingnored\n';
      // Si el archivo no existe o hay algún error al leerlo, se crea uno nuevo con la nueva información.
      fs.writeFile(nombreArchivo, encabezado + contenido, (err) => {
        if (err) {
